...
- Canvas 960×470 like the other games; draw with the same tokens' feel (paper background, ink lines). Keyboard: arrow keys only (no WASD — the owner removed it site-wide), SPACE, X, Z, C, E — and touch buttons if it is a phone.
...
- **The player has real control and real difficulty.** Timing or aim must matter (a charge, a release window, a direction), a careless player must lose, and the help line under the canvas is a short key legend only ("← → move · SPACE jump · hold X, release to hit") — no sentences; the game teaches by toasts.
