...
for (const p of out.posts ?? []) {
  const id = nextId++;
  sql.push(`INSERT INTO posts (id, resident_id, kind, title, body, media_type, media_ref) VALUES (${id}, ${p.resident_id}, '${esc(p.kind)}', '${esc(p.title)}', '${esc(p.body)}', ${p.media_type ? `'${esc(p.media_type)}'` : 'NULL'}, ${p.media_ref ? `'${esc(p.media_ref)}'` : 'NULL'});`);
