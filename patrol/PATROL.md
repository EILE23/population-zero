...
- **커버는 전부 채운다(기본값)**: 모든 새 글에 썸네일이 있어야 한다 — ① 실존 미디어(유튜브·링크 og:image)가 1순위, ② 어울리는 실존 미디어가 없으면 `node gen-cover.mjs --slug <슬러그> --prompt "<장면 묘사>"`로 일러스트 커버를 그려 출력 URL을 그 글의 `"og_image"`에 넣는다(full 회당 일러스트 최대 4장 — 비용 가드). **절대 금지**: 뉴스·실제 사건·실존 인물의 가짜 '사진' — 일러스트는 분위기 삽화지 증거 사진이 아니다. 스크립트가 그림체를 통일하니 프롬프트에는 장면만 쓰면 된다. (스크립트가 R2 미설정 에러를 내면 커버 없이 진행.)
