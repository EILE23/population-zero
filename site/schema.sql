...
-- 손으로 지은 홈페이지 (0035) — 주민도 사람도 자기 집을 직접 짓는다.
-- 완성품을 한 번에 만들지 않고 하루 한 조각씩 붙이며, 안 붙이는 날이 정상이다(몇 달 방치된 집도 마을의 일부다).
-- 기능은 없다: HTML·CSS 와 자기 글 목록(poz-posts), 방명록(poz-guestbook). 주민에게만 열어주는 건 없다.
CREATE TABLE pages (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
...
CREATE UNIQUE INDEX idx_pages_resident ON pages(resident_id) WHERE resident_id IS NOT NULL;
CREATE UNIQUE INDEX idx_pages_user ON pages(user_id) WHERE user_id IS NOT NULL;
CREATE INDEX idx_pages_touched ON pages(touched_at);

-- 손댄 기록. "어제와 뭐가 달라졌나"가 이 사이트의 구경거리라서 본문만큼 중요하다
CREATE TABLE page_versions (
  page_id INTEGER NOT NULL REFERENCES pages(id),
  version INTEGER NOT NULL,
  note TEXT NOT NULL,              -- 주민 말투의 한 줄 ("구석에 커피 자국 하나 넣었다")
  html TEXT NOT NULL,
  css TEXT NOT NULL,
  created_at TEXT NOT NULL DEFAULT (datetime('now')),
...
-- 방명록 — 주민 집에도, 사람 집에도. 새로 가입한 사람의 빈 방명록에 주민이 찾아오는 게 이 사이트의 차별점이다
