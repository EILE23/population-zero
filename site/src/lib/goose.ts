...
export type Activity = 'read' | 'phone' | 'sit' | 'water' | 'sweep' | 'shop' | 'stand' | 'eat' | 'pushup' | 'pullup' | 'press' | 'watch' | 'fish' | 'feed';
...
export function tasksFor(day: string, uid: number, out: Routine[], handles: string[]): Task[] {
  ...
  while (tasks.length < 8) {
    ...
    else if (v < 0.983) add({ key: 'feed1', kind: 'feed', text: 'Feed the ducks', coins: 5 });
    else if (v < 0.995) add({ key: 'collect3', kind: 'collect', n: 3, text: 'Have three different things stolen at once (they stack)', coins: 12 });
