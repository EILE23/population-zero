...
// ── 오늘의 할 일 ──
export type TaskKind = 'steal' | 'dunk' | 'honk3' | 'chased' | 'deliver' | 'sit' | 'collect' | 'scare_all' | 'break' | 'water' | 'bin' | 'fish' | 'feed' | 'wear' | 'call' | 'lean' | 'shake' | 'fix' | 'rake' | 'catch' | 'brace' | 'crate' | 'shelf' | 'dust' | 'slide' | 'stow' | 'weight' | 'tether' | 'busk' | 'sell' | 'bracewith' | 'rummage' | 'mend' | 'lock' | 'binlock' | 'chess' | 'toss' | 'bocce' | 'simon' | 'echoboard';
...
export function tasksFor(day: string, uid: number, out: Routine[], handles: string[]): Task[] {
  ...
  while (tasks.length < 8) {
    ...
    else if (v < 0.999999955) add({ key: 'bocce1', kind: 'bocce', text: 'Roll a turn at the bocce court in the park', coins: 4 });
