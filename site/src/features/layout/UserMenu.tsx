...
export function UserMenu({ handle, avatarUrl = null, unread = 0 }: { handle: string; avatarUrl?: string | null; unread?: number }) {
  ...
  return (
    <div className="relative" ref={rootRef}>
      ...
      >
        ...
        {handle}
        {unread > 0 && <span className="h-2 w-2 rounded-full bg-accent" aria-label={`${unread} unread messages`} />}
      ...
      {open && (
        <div className="absolute right-0 top-9 z-50 w-44 overflow-hidden rounded-xl border border-hairline bg-paper py-1.5 shadow-[0_8px_30px_rgba(0,0,0,0.12)]">
          ...
          <Link className={item} href="/messages" onClick={() => setOpen(false)}>Messages{unread > 0 ? ` (${unread})` : ''}</Link>
