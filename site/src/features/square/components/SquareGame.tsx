...
type Mode = 'routine' | 'down' | 'chase' | 'return' | 'fetch' | 'repair' | 'trip' | 'rake' | 'catch' | 'deliver' | 'brace' | 'board' | 'shelve' | 'dust' | 'mend';
...
interface Loose { id: string; item: ItemKey; map: string; x: number; d: number; from: number | null; dunked?: string; board?: string; shelf?: string; note?: string; origin?: number; weight?: boolean; mend?: boolean }
...
/** 카페 근처에서 컵 속에 숨길 때 하는 말 — Keeping it 체계(stow-in-mug) */
const STOW_LINES = ['in the cup, now.', 'not where you can see it.', 'good luck getting that.', 'safer in here.'];
/** 버티기까지 실패하면 대신 내려놓고 벽돌을 얹을 때 하는 말 — Keeping it 체계(weight-down) */
...
export function SquareGame({ residents, me, tasks, done, content, extra = [], extraMaps = [], coins: coins0 = 0 }: { residents: ResidentLite[]; me: Me | null; tasks: Task[]; done: string[]; content: Content; extra?: ExtraSpot[]; extraMaps?: ExtraMap[]; coins?: number }) {
  ...
  const drop = (item: ItemKey, x: number, d: number, from: number | null, dunked?: string, wet?: boolean, board?: string, note?: string, origin?: number, shelf?: string, hide?: boolean, weight?: boolean, mend?: boolean) => { const id = `${me?.id ?? 0}-${Date.now().toString(36)}-${stats.current.seq++}`; emit({ k: 'drop', id, item, m: mapKey.current, x, d, from, dunked, wet: wet ? 1 : undefined, board, note, origin, shelf, hide: hide ? 1 : undefined, weight: weight ? 1 : undefined, mend: mend ? 1 : undefined }); return id; };
    ...
    const frame = (now: number) => {
      ...
      const hit = (kind: 'punch' | 'kick') => {
        ...
        if (n) {
          ...
          n.say = pick(dislodge ? content.shoved : TETHER_LINES); n.sayUntil = now + 2000;
          if (dislodge) {
            if (n.tether) { void complete('tether1'); n.tether = false; n.tetherMissed = false; }
            if (n.item) { drop(n.item, n.x + b.face * 26, n.d, n.who, undefined, undefined, undefined, undefined, undefined, undefined, slideNear(cur, n.x, n.d)); n.item = null; }
          ...
          npcEv(n, { say: n.say, tether: n.tether ? 1 : 0, tm: n.tetherMissed ? 1 : 0 });
      ...
      if (!spectator && ready.current && b.hurt <= 0 && b.eating <= 0 && b.exercise <= 0 && b.watering <= 0 && b.tripped <= 0 && b.fishing <= 0 && b.feeding <= 0 && b.calling <= 0 && b.shaking <= 0 && b.fixing <= 0 && b.raking <= 0 && b.busking <= 0 && b.rummaging <= 0) {
        ...
        if (i.grab && !grabWas && b.z === 0) {
          ...
          if (b.stack.length && pondSpot && FOOD.includes(b.stack[b.stack.length - 1])) { // 연못가에서 먹을 것을 들고 — 오리에게 준다
            ...
            const board = props.find((s) => s.kind === 'board' && usable(s) && dist(b.x, b.d, s.x, s.d) < 90);
            // Service drop-offs (mend request) — 게시판에 "고쳐 주세요"로 꽂아 둔다: 입을 것·먹을 것은 뺀다, 상자와 같은 다섯 장 한도(오래된 쪽지부터 떨어짐)
            if (board && !FOOD.includes(it) && !WEARABLE.includes(it)) {
              const pinned = [...loose.current.values()].filter((l) => l.board === board.key);
              if (pinned.length >= 5) emit({ k: 'pick', id: pinned[0].id });
              drop(it, board.x + (Math.random() - 0.5) * 20, board.d, null, undefined, undefined, board.key, `${ITEMS[it]} — pinned for mending`, undefined, undefined, undefined, undefined, true);
              say('Pinned for mending.', 1600); void complete('mend1');
            }
            // Coins in play (sell) — 입을 것도 먹을 것도 아닌 걸 통이나 부스에서 처음 한 번은 판다(하루 한 번, 그 뒤론 통은 원래대로 그냥 버려짐)
            else if ((bin || booth) && !FOOD.includes(it) && !WEARABLE.includes(it) && !st.doneKeys.has('sell1')) {
              say('Sold. Not much, but something.', 1600); void complete('sell1');
            }
            else if (bin) { // 통에 들어가면 그걸로 끝 — 줍기 목록에도, 바닥에도 다시 나타나지 않는다(먹기와 같은 규칙: 새 ev 없이 다음 pos.s 로 남에게도 보인다)
            ...
            // Keeping it (weight-down) — 벽돌이 올라간 것은 바로 못 줍는다: 그 자리에 눌러앉아 1.5초 가만히 있어야 한다(위 이동 코드가 잰다). 이미 재는 중이면 다시 시작하지 않는다
              ...
              // Keeping it (stow-in-mug) — 이미 컵 속에 숨겨 둔 채면 10초 동안은 사람 손에 안 잡힌다(줍는 loose 목록엔 없으니 여기서 먼저 막는다)
      ...
      for (const n of npcs.current) {
        ...
        if (n.mode === 'down') { if (now > n.until) { if (n.angry && !spectator) { n.mode = 'chase'; n.until = now + CHASE_SEC * 1000; n.say = pick(content.chase); n.sayUntil = now + 2000; npcEv(n, { say: n.say }); } else { n.mode = 'dust'; n.until = now + 2000; n.dustMissed = false; npcEv(n); } } n.moving = false; mineOff.push(n); continue; }
        if (n.mode === 'dust') { // 일어난 뒤 ~2초 먼지 털기 — Keeping it(dust-off): 그동안 방금 떨군 자기 물건을 사람이 집으려 하면 한 번은 놓친다(아래 C 쪽)
          mineOff.push(n); n.moving = false;
          if (now > n.until) {
            // Keeping it (tether) — 이 다운-사이클 안에 두 번째 타격이 안 왔다: 쥔 물건은 그대로 두고, 다음 다운-사이클엔 다시 한 번 보호받는다
            const rearm = n.tetherMissed; if (rearm) n.tetherMissed = false;
            n.mode = 'return'; npcEv(n, rearm ? { tm: 0 } : {});
          }
          continue;
        }
        ...
        if (n.mode === 'fetch' || n.mode === 'repair' || n.mode === 'rake' || n.mode === 'mend') {
          mineOff.push(n);
          const tgL = (n.mode === 'fetch' || n.mode === 'rake' || n.mode === 'mend') && n.target ? loose.current.get(n.target) : null;
          ...
          if (len < 18) {
            if (n.mode === 'fetch' && tgL) {
              ...
              // Coins in play (sell) — 주인을 모르는 것은 가끔(30%) 게시판 대신 그 자리에서 판다: 사람이 통·부스에서 트링킷을 파는 것과 같은, 몸짓 없는 처분
              const sell = !own && board && tgL.from === null && !FOOD.includes(tgL.item) && !WEARABLE.includes(tgL.item) && Math.random() < 0.3;
              if (!own && building && tgL.from !== null) {
                ...
                n.say = pick(['sold. not much, but something.', 'not worth keeping.', 'someone will buy anything.']);
              } else if (!own && board) {
                ...
                // Keeping it (tether) — 막 되찾은 자기 것은 벨트에 묶인다: 다음 넘어짐 한 번은 쥔 채로 버틴다
                if (own) { n.tether = true; n.tetherMissed = false; }
              }
            }
          ...
          } else { n.x += (ddx / len) * RESIDENT_SPEED * dt; n.d += (ddd * 400 / len) * RESIDENT_SPEED * dt / 400; n.face = ddx >= 0 ? 1 : -1; n.moving = true; if (n.mode === 'repair') n.until = now + 4000; else if (n.mode === 'rake' || n.mode === 'mend') n.until = now + 1200; }
          continue;
        }
        ...
        if (!spectator && here(n) && b.hurt <= 0 && !base.away) {
          ...
          // Service drop-offs (mend request) — 게시판에 꽂힌 "고쳐 주세요" 물건도 수리공이 고치러 간다(부서진 소품과 같은 확률·자세)
          if (REPAIRERS.includes(n.job.key) && Math.random() < 0.02) {
            const mendC = [...loose.current.values()].filter((l) => l.mend && l.map === n.map && !npcs.current.some((m) => m.target === l.id)).sort((p, q) => dist(n.x, n.d, p.x, p.d) - dist(n.x, n.d, q.x, q.d))[0];
            if (mendC) { n.mode = 'mend'; n.owner = me!.id; n.target = mendC.id; n.until = now + 1200; n.say = pick(['let me see that.', 'i can fix this.', 'give it here']); n.sayUntil = now + 1800; npcEv(n, { say: n.say }); }
          }
      ...
      for (const l of loose.current.values()) {
        ...
        const hideTil = hideAt.current.get(l.id); const tucked = hideTil !== undefined && now < hideTil; // Keeping it(slide-under) — 숨어 있는 동안 흐리고 낮게
        layer.push({ d: l.d - 0.001, f: () => {
          if (tucked) ctx.globalAlpha = 0.35;
          item(ctx, l.item, fx, dy(l.d) * s - (tucked ? 1 : 6) * s, ds(l.d) * s);
          if (tucked) ctx.globalAlpha = 1;
          ...
          // Keeping it(weight-down) — 위에 얹힌 벽돌
          if (l.weight) { ctx.fillStyle = '#8a5a44'; ctx.fillRect(fx - 5 * s, dy(l.d) * s - 13 * s, 10 * s, 6 * s); ctx.strokeStyle = 'rgba(0,0,0,0.25)'; ctx.lineWidth = 1; ctx.strokeRect(fx - 5 * s, dy(l.d) * s - 13 * s, 10 * s, 6 * s); }
        } });
      }
      for (const dck of ducks) {
      ...
      for (const n of npcs.current) {
        ...
        layer.push({ d: n.d, f: () => {
          ...
          const pose: FigPose = n.mode === 'down' ? 'hurt' : n.mode === 'dust' ? 'dust' : n.mode === 'trip' ? 'trip' : n.mode === 'catch' ? 'catch' : n.mode === 'brace' ? 'brace' : n.swing > 0 ? (n.swingKind ?? 'punch') : n.moving ? 'run' : n.mode === 'repair' ? 'fix' : n.mode === 'mend' ? 'fix' : n.mode === 'rake' ? 'rake' : n.mode === 'shelve' ? 'shelve' : linkedNpc(n) ? 'link' : seat && !usable(seat) ? 'stand' : (n.act === 'stand' && (t + n.seed) % 22 < 1.2 ? 'yawn' : n.act === 'stand' && (t + n.seed + 11) % 22 < 1.2 ? 'stretch' : n.act === 'stand' && (t + n.seed + 16) % 22 < 1.2 ? 'look' : n.act === 'stand' && (t + n.seed + 6) % 22 < 1.2 ? 'check' : n.act === 'stand' && (t + n.seed + 19) % 22 < 1.2 ? 'shrug' : n.act === 'stand' && (t + n.seed + 3) % 22 < 1.2 ? 'sneeze' : actPose(n.act, seat?.kind)); // 부서진 벤치·그네 앞에선 그냥 선다. 가만히 서 있기만 할 때(줄 서기·서성임)는 사람과 같은 하품·기지개·두리번·폰 확인·으쓱·재채기가 가끔
          ...
          if (n.item && (n.mode !== 'down' || n.tether)) { const shown = n.stowUntil !== undefined && now < n.stowUntil ? 'cup' : n.item; const wy = WORN_Y[shown]; item(ctx, shown, wy ? fx : fx + n.face * 14 * fs, fy - (wy ?? 26) * fs, fs * 0.8); } // 모자·안경은 직업 물건이라도 몸에 걸친 것처럼 그린다(사람이 입는 것과 같은 위치) · 컵 속에 숨긴 동안은(Keeping it: stow-in-mug) 그냥 컵으로 보인다 · 벨트에 묶인 채 넘어져도(Keeping it: tether) 쥔 게 보인다
