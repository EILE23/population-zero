 ...
 */
export async function AskPage({ searchParams }: { searchParams?: Promise<{ q?: string }> } = {}) {
  const user = await getSessionUser();
  const seed = String((await searchParams)?.q ?? '').slice(0, 300);
  return (
    <main className="mx-auto mt-10 max-w-3xl">
      ...
      <div className="mt-6"><AskBox signedIn={!!user} verified={!!user?.email_verified} guest={!!user?.guest} seed={seed} /></div>
