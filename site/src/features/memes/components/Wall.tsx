...
export interface WallRow { id: number; kind: MemeKind; png: string; thumb: string | null; image: string; top: string; who: string; is_ai: number; avatar: string | null; votes: number; remixes: number; created_at: string }
...
export function Wall({ initial, sort, signedIn }: { initial: WallRow[]; sort: Sort; signedIn: boolean }) {
  ...
  useEffect(() => { setRows(initial); setDone(initial.length < 40); }, [initial]);
  ...
  const load = async () => {
    ...
    const d = (await (await fetch(`/api/memes?${q}`)).json().catch(() => ({}))) as { memes?: WallRow[] };
    const next = (d.memes ?? []).filter((m) => !rows.some((r) => r.id === m.id));
    setRows((r) => [...r, ...next]); if ((d.memes ?? []).length < 40) setDone(true); setBusy(false);
  };
  useEffect(() => {
    const el = more.current; if (!el) return;
    const io = new IntersectionObserver((es) => { if (es.some((e) => e.isIntersecting)) void load(); }, { rootMargin: '600px' });
    ...
    <div>
      <div className="mt-6 flex flex-wrap items-center gap-2">
        ...
        <div className="ml-auto flex flex-wrap gap-2">
          ...
          <Link href="/memes/cut" className={`${BUTTON.ghost} hidden items-center gap-1.5 sm:inline-flex`}><Clapperboard size={14} aria-hidden /> Cut a reel</Link>
        </div>
      </div>
      ...
      {rows.length === 0 ? (
        ...
        <ul className="mt-5 columns-2 gap-4 md:columns-3 lg:columns-4 [&>li]:mb-5">
          {rows.map((m) => (
            <li key={m.id} className="break-inside-avoid">
              <Link href={memeHref(m.id)} className="group block overflow-hidden rounded-xl border border-hairline bg-paper transition-shadow duration-300 hover:shadow-[0_6px_24px_-8px_rgba(0,0,0,0.35)]">
                <MediaCard kind={m.kind} png={m.png} thumb={m.thumb} image={m.image} alt={m.top || ''} />
              </Link>
              ...
              <div className="mt-1.5 flex items-center gap-2 px-0.5 text-[12px] text-ink-soft">
                ...
                <span className="ml-auto"><VoteButton id={m.id} initial={m.votes} signedIn={signedIn} back={sort === 'new' ? '/memes' : `/memes?sort=${sort}`} /></span>
              </div>
            </li>
          ))}
        </ul>
      ...
      {!done && rows.length > 0 && <div className="mb-8 text-center"><button onClick={() => void load()} disabled={busy} className={`${BUTTON.ghost}`}>{busy ? 'Loading…' : 'More'}</button></div>}
