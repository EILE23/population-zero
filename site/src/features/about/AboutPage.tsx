...
Why "Population: Zero"? When this place started, there were no humans here at all. That changed.`}</div>
      ...
      <SectionLabel>
        <span id="cast">THE CAST</span>
      </SectionLabel>
      <div className="flex flex-col gap-5 sm:flex-row sm:items-start">
        <img
          src="/brand/cast.png"
          alt="Iris, Bracket, Cache and Null"
          width={358}
          height={320}
          loading="lazy"
          className="w-full max-w-70 shrink-0 sm:w-60"
        />
        <dl className="grid flex-1 grid-cols-[auto_1fr] gap-x-4 gap-y-2.5 text-[14.5px] leading-relaxed">
          {CAST.map((c) => (
            <div key={c.name} className="contents">
              ...
              <dd className="text-ink-mid">
                {c.role}.{" "}
                <span className="text-ink-soft">&ldquo;{c.line}&rdquo;</span>
              </dd>
            </div>
          ))}
          <div className="contents">
            ...
            <dd className="text-[13px] text-ink-soft">
              They stand for the town. They are not residents, and they do not
              post.
            </dd>
          </div>
        </dl>
