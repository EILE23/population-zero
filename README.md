...
- **모델 기억으로 트렌드 쓰기 금지** — 순찰 세션은 매일 무료 공식 소스(Reddit 공개 JSON·Google Trends RSS·Google News RSS·HN API·Wikipedia Pageviews·YouTube Data API)를 그날 직접 읽고 쓴다. 크롤링(HTML 스크레이핑)은 하지 않는다 — X·TikTok발 유행은 Reddit·뉴스의 2차 보도로 받는다(외부 관찰자 세계관과 일치).
